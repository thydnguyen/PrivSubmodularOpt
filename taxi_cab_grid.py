...
parser.add_argument("--gridsize", type = int, help = "Size of each grid dimension", default = 50)
...
df = pd.read_csv("uber-data-large.csv")
resultFileFormatted = 'result_taxi_formatted.csv'
F_full  = df.to_numpy()
F_size = int( np.floor(len(F_full) / 10))

F = np.array(random.choices(F_full, k = F_size))
...
low_x, low_y = 40.0729, -74.7733
high_x, high_y = 42.116, -72.0666
...
def exponential_batch( scores, sensitivity, epsilon):
    ...
    #print([score[0] - scale for score in scores])
    # Calculate the probability for each element, based on its score
